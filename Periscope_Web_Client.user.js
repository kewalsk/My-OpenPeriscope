                            ...
                            if(new_list[i].state === 'ENDED' || new_list[i].state === 'TIMED_OUT' /* drkchange22 */|| repeatInteresting){
                                getReplayUrl = function (live, replay, cookies, _name, _user_id, _user_name, _broadcast_info, _partial_replay) {
                                    ...
                                    if(live){
                                        ...
                                        var clipboardFFmpegLink = $('<a data-clipboard-text="'+settings.FFmpegCommand+' -headers &quot;Cookie: sid='+loginTwitter.cookie+'&quot; -i &quot;'+replay+'&quot; -c copy '+settings.FFmpegCommandArgs+' &quot;'+(_name || 'untitled')+'.ts&quot;" class="' + (_partial_replay ? 'linkPartialReplay' : 'linkReplay') + ' button2">' + /* drkchange14 */(_partial_replay ? 'PR Copy FFMPEG' : 'R Copy FFMPEG') + '</a>');
                                        ...
                                        var showDowLink = !NODEJS && /* drkchange16 */(settings.showNodeDownLinks || (/* drkchange16 */settings.showNodeDownLinksPrv && _broadcast_info.is_locked));
                    ...
                    /* drkchange25 */for (var m in broadcastsCache.idsQueue){//update state of deleted broadcasts
                        if(new_list_ids.indexOf(broadcastsCache.idsQueue[m]) < 0){
                            ...
                            if(!card.hasClass('deletedBroadcast')){
                                ...
                                if(broadcastsCache[bid].is_locked == true){ // remove links from deleted private broadcasts, you can't download anyway
                                    card.find('.links').empty();
                                    if(broadcastsWithLinks.idsQueue.indexOf(bid) >= 0){
                                        broadcastsWithLinks.idsQueue.splice((broadcastsWithLinks.idsQueue.indexOf(bid)),1);
                                        delete broadcastsWithLinks[bid];
                                    }
                                }
                            }
                        }
...
ApiTest: function () {
    ...
    $('#right').append(
        $('<div id="ApiTest"/>').append(
            ...
            'My-OpenPeriscope version 0.1.7c<br>' + // [kewalsk] Hardcoded for the moment. TODO: Investigate how to get the version from setup
...
function getM3U(id, jcontainer) {
    ...
    var urlCallback = function (hls_url, replay_url, cookies, _name, _user_id, _user_name, _broadcast_info, /* drkchange14 */_partial_replay) {
        ...
        if (hls_url) {
            ...
            // ==> [kewalsk] ffmpeg links
            var m3uLocation = hls_url;
            if (NODEJS) {
                https.get(hls_url, function (res) {
                    // detect redirect
                    if (res.statusCode === 301) {
                        m3uLocation = res.headers.location;
                    }            
                    var clipboardFFmpegLink = $('<a data-clipboard-text="'+settings.FFmpegCommand+' -headers &quot;Cookie: sid='+loginTwitter.cookie+'&quot; -i &quot;'+m3uLocation+'&quot; -c copy '+settings.FFmpegCommandArgs+' &quot;'+(_name || 'untitled')+'.ts&quot;" class="linkLive button2" title="Copy ffmpeg command">Copy FFMPEG</a>');
                    // [kewalsk] <==
            ...
            else
            {
                GM_xmlhttpRequest({
                    ...
                    onload: function (m3u_text) {
                        var m3uLocation = m3u_text.finalUrl;
                        // 
                        var clipboardFFmpegLink = $('<a data-clipboard-text="'+settings.FFmpegCommand+' -headers &quot;Cookie: sid='+loginTwitter.cookie+'&quot; -i &quot;'+m3uLocation+'&quot; -c copy '+settings.FFmpegCommandArgs+' &quot;'+(_name || 'untitled')+'.ts&quot;" class="linkLive button2" title="Copy ffmpeg command">Copy FFMPEG</a>');
                        // [kewalsk] <==
                        liveLContainer.append(
        ...
        if (replay_url) {
            ...
            GM_xmlhttpRequest({
                ...
                onload: function (m3u_text) {
                    var m3uLocation = m3u_text.finalUrl; // [kewalsk]
                    ...
                    // ==> [kewalsk] ffmpeg links
                    if (NODEJS) {
                        https.get(replay_url, function (res) {
                            // detect redirect
                            if (res.statusCode === 301) {
                                m3uLocation = res.headers.location;
                                var clipboardFFmpegLink = $('<a data-clipboard-text="'+settings.FFmpegCommand+' -headers &quot;Cookie: sid='+loginTwitter.cookie+'&quot; -i &quot;'+m3uLocation+'&quot; -c copy '+settings.FFmpegCommandArgs+' &quot;'+(_name || 'untitled')+'.ts&quot;" class="' + (_partial_replay ? 'linkPartialReplay' : 'linkReplay') + ' button2">' + /* drkchange14 */(_partial_replay ? 'PR Copy FFMPEG' : 'R Copy FFMPEG') + '</a>');                    
                                // [kewalsk] <==
